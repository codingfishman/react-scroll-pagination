(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["ReactScrollPagination"] = factory(require("React"));
	else
		root["ReactScrollPagination"] = factory(root["React"]);
...
var ReactScrollPagination = function (_Component) {
  (0, _inherits3.default)(ReactScrollPagination, _Component);

  function ReactScrollPagination() {
    var _ref;

    var _temp, _this, _ret;

    (0, _classCallCheck3.default)(this, ReactScrollPagination);

    for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }

    return _ret = (_temp = (_this = (0, _possibleConstructorReturn3.default)(this, (_ref = ReactScrollPagination.__proto__ || (0, _getPrototypeOf2.default)(ReactScrollPagination)).call.apply(_ref, [this].concat(args))), _this), _this.isolate = {
      onePageHeight: null,
    ...
    }, _this.showPagePositionDiv = function () {
      if (_this.isolate.timeoutFuncHandler) {
        clearTimeout(_this.isolate.timeoutFuncHandler);
      }
      _this.setState({ showPageStatus: true });

      _this.isolate.timeoutFuncHandler = setTimeout(function () {
        _this.setState({ showPageStatus: false });
      }, _this.isolate.showTime);
    }, _this.getShowTime = function () {
      var showTime = _this.isolate.defaultShowTime;
      if (_this.props.paginationShowTime) {
        showTime = parseInt(_this.props.paginationShowTime);
        if (isNaN(showTime)) {
          showTime = _this.isolate.defaultShowTime;
          console.error('WARNING: Failed to convert props "paginationShowTime" to Number with value: "' + _this.props.paginationShowTime + '". Will take ' + _this.isolate.defaultShowTime + ' by default.');
        }
      }
      return showTime;
    }, _this.getExcludeHeight = function () {
      // 获取需要减去的高度
      var excludeHeight = _this.isolate.defaultExcludeHeight;

      if (_this.props.excludeHeight) {
        var propsExcludeHeight = parseInt(_this.props.excludeHeight);
        if (isNaN(propsExcludeHeight)) {
          console.error('WARNING: Failed to convert the props "excludeHeight" with value: "' + _this.props.excludeHeight + '" to Number, please verify. Will take "' + _this.isolate.defaultExcludeHeight + '" by default.');
        } else {
          excludeHeight = propsExcludeHeight;
        }
      } else if (_this.props.excludeElement && typeof _this.props.excludeElement === 'string') {
        var excludeEle = document.querySelector(_this.props.excludeElement);

        if (excludeEle) {
          excludeHeight = excludeEle.offsetHeight;
        } else {
          console.error('WARNING: Failed to get the element with given selector "' + _this.props.excludeElement + '", please veirify. Will take "' + _this.isolate.defaultExcludeHeight + '" by default.');
        }
      }
      _this.isolate.excludeHeight = excludeHeight;

      return excludeHeight;
    }, _this.getTriggerAt = function () {
      var triggerAt = _this.isolate.defaultTrigger;

      if (_this.props.triggerAt) {
        triggerAt = parseInt(_this.props.triggerAt);

        if (isNaN(triggerAt)) {
          triggerAt = _this.isolate.defaultTrigger;

          console.error('WARNING: Failed to convert the props "triggerAt" to number with value: "' + _this.props.triggerAt + '". Will take 30px by default.');
        }
      }

      return triggerAt;
    }, _this.getOnePageHeight = function () {
      ...
      */
      if (typeof _this.props.totalPages === 'number' && _this.props.totalPages > 0 && _this.isolate.onePageHeight === null) {
        _this.isolate.onePageHeight = documentHeight - _this.isolate.excludeHeight;
      }
    }, _this.handlePagePosition = function () {
      _this.getOnePageHeight();

      var scrollBottom = _this.scrollTop() + _this.windowHeight() - _this.isolate.excludeHeight;

      if (_this.isolate.onePageHeight !== null) {
        var currentPage = Math.ceil(scrollBottom / _this.isolate.onePageHeight) || 1;
        _this.setState({ currentPage: currentPage });
        _this.showPagePositionDiv();
      }
    }, _this.windowHeight = function () {
      return Math.max(document.body.clientHeight, window.innerWidth);
    }, _this.documentHeight = function () {
      return Math.max(_this.windowHeight(), document.documentElement.clientHeight, document.body.scrollHeight, document.documentElement.scrollHeight, document.body.offsetHeight, document.documentElement.offsetHeight);
    }, _this.scrollTop = function () {
      return window.pageYOffset || document.documentElement.scrollTop;
    }, _this.scrollHandler = function () {
      var scrollBottom = _this.scrollTop() + _this.windowHeight();
      var triggerBottom = scrollBottom + _this.isolate.triggerAt;

      // 当滚动条距离底部距离小于30像素的时候出发请求操作
      if (triggerBottom >= _this.documentHeight()) {
        _this.props.fetchFunc();
      }

      _this.handlePagePosition();
    }, _this.validateAndSetPropValues = function () {
      _this.isolate.triggerAt = _this.getTriggerAt();
      _this.isolate.excludeHeight = _this.getExcludeHeight();
      _this.isolate.showTime = _this.getShowTime();
    }, _this.componentWillUnmount = function () {
      window.removeEventListener('scroll', _this.scrollHandler);
    }, _this.componentDidMount = function () {
      _this.validateAndSetPropValues();
      window.addEventListener('scroll', _this.scrollHandler);
    }, _this.render = function () {
      // if no totalPages presented, will only do the fetchings
      if (typeof _this.props.totalPages === 'undefined') {
        return null;
      }

      var acutalPageContentDivStyle = (0, _assign2.default)({}, _this.props.innerDivStyle || _this.pageContentStyle);

      // always set the opacity for inner div, so they are able to make the transition
      if (!_this.state.showPageStatus) {
        acutalPageContentDivStyle.opacity = 0;
      }

      return React.createElement(
        'div',
        { style: _this.props.outterDivStyle || _this.pageDivStle },
        React.createElement(
          'div',
  ...
  return ReactScrollPagination;
}(_react.Component);

ReactScrollPagination.defaultProps = {
  totalPages: null
};
ReactScrollPagination.propTypes = {
